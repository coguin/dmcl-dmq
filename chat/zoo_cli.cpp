...
int watcher(zhandle_t *zzh,int type,int state,const char*path,int *watcherCtx){
	std::cout<<"zookeeper connected"<<endl;
	//std::cout<<"Watcher "<<type2String(type) <<"state ="<<state2String(state);
}
int my_string_completion_free_data(int rc, const char *name, const int *data){
 ...
 * ************/
int Zoo_cli::register_broker(char * br_name){
	char p[30]="/brokers/";
	strcat(p,br_name);
//	 char str[]="/xyz6";
 ...
 * ********************/
int Zoo_cli::delete_topic(char *topic){
	char p[50]="/topics/";
	strcat(p,topic);
 ...
 * **************/
int Zoo_cli::delete_topic_consumer(char * consumer,char * topic){
	vector<string> consumers;
	char p[50]="/topics/";
	strcat(p,topic);
	strcat(p,"/");
...
/*************consumer***************/
int Zoo_cli::register_consumer(char * con_name){
...
int Zoo_cli::delete_consumer(char *consumer){
	char p[50]="/consumers/";
	strcat(p,consumer);
	int rc = zoo_delete(zh,p,-1);
	if(rc){
 ...
 * ************/
int Zoo_cli::sub_(char * consumer,char* topic){
	char p[50]="/topics/";
	strcat(p,topic);
	strcat(p,"/");
	strcat(p,consumer);
