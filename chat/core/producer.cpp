...
int main(int argc, char* argv[])
{
    try
    {
        if(argc != 3)
        {
            ...
            std::cerr << "Usage: chat_client <host> <topic>\n";
            return 1;
        }
        {
            ...
            // 和server通信，告知server自己的topic
            chat_message init;
            init.body_length(std::strlen(argv[2]) + 1);
            init.body()[0] = '1';
            std::memcpy(init.body() + 1, argv[2], init.body_length() - 1);
            init.encode_header();
            c.write(init);
        ...
        auto endpoint_iterator = resolver.resolve({ broker_ip, "20000"});
        chat_client c(io_service, endpoint_iterator);
